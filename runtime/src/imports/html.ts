...
import { isCheerioNode, type GlobalStore } from "../global-store";

// HTML error codes matching aidoku-rs HtmlError
const HtmlError = {
  InvalidDescriptor: -1,
  InvalidString: -2,
  InvalidHtml: -3,
...
export function createHtmlImports(store: GlobalStore) {
  return {
    ...
    parse: (
      ...
      const baseUrl = baseUrlLen > 0 ? store.readString(baseUrlPtr, baseUrlLen) : undefined;
    ...
    select: (descriptor: number, selectorPtr: number, selectorLen: number): number => {
    ...
    select_first: (descriptor: number, selectorPtr: number, selectorLen: number): number => {
      if (descriptor < 0) return HtmlError.InvalidDescriptor;
      if (selectorLen <= 0) return HtmlError.InvalidString;
      const node = getNode(store, descriptor);
      if (!node) return HtmlError.InvalidDescriptor;
    ...
    set_text: (descriptor: number, textPtr: number, textLen: number): number => {
      if (descriptor < 0) return HtmlError.InvalidDescriptor;
      const node = getNode(store, descriptor);
      if (!node) return HtmlError.InvalidDescriptor;
      const text = store.readString(textPtr, textLen);
    ...
    set_html: (descriptor: number, htmlPtr: number, htmlLen: number): number => {
      if (descriptor < 0) return HtmlError.InvalidDescriptor;
      const node = getNode(store, descriptor);
      if (!node) return HtmlError.InvalidDescriptor;
    ...
    prepend: (descriptor: number, htmlPtr: number, htmlLen: number): number => {
      if (descriptor < 0) return HtmlError.InvalidDescriptor;
      const node = getNode(store, descriptor);
      if (!node) return HtmlError.InvalidDescriptor;
      const html = store.readString(htmlPtr, htmlLen);
    ...
    has_class: (descriptor: number, classPtr: number, classLen: number): number => {
      const node = getNode(store, descriptor);
    ...
    has_attr: (descriptor: number, attrPtr: number, attrLen: number): number => {
      const node = getNode(store, descriptor);
      const attrName = store.readString(attrPtr, attrLen);
