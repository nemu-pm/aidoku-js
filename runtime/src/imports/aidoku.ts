/**
 * aidoku namespace - Aidoku-specific object creation (OLD ABI)
 */
import { isArray, type GlobalStore } from "../global-store";
import type {
  Manga,
  Chapter,
  Page,
  ...
  return {
    ...
    create_chapter: (
      ...
      const chapterObj: Chapter = {
        ...
        scanlator:
          scanlatorLen > 0 ? store.readString(scanlatorPtr, scanlatorLen) || undefined : undefined,
        url: urlLen > 0 ? store.readString(urlPtr, urlLen) || undefined : undefined,
    ...
    create_deeplink: (mangaDesc: number, chapterDesc: number): number => {
      const deepLink: DeepLink = {
        ...
        chapter:
          chapterDesc > 0 ? (store.readStdValue(chapterDesc) as Chapter | undefined) : undefined,
      };
